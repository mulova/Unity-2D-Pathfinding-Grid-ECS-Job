...
namespace Pathfinding
{
    public class InitializePathfinding : MonoBehaviour
    {
        ...
        private void Awake()
        {
            World.DefaultGameObjectInjectionWorld.GetExistingSystem<PathfindingSystem>().canMoveDiag = canMoveDiag;
            CreateGrid();
        }    
        ...
        public void CreateBlockedNodes()
        {
            ...
            var cells = RequiredExtensions.nodes;

            for (int i = 0; i < numbOfRandomBlockedNodes; i++)
            {
                int randomCell = Random.Range(0, size.x * size.y);

                cells[randomCell] = new Node { obstacle = Convert.ToByte(true), Height = 0 };
            }
        }
        ...
        public void CreateGrid()
        {
            ...
            for (int y = 0; y < size.y; y++)
            {
                for (int x = 0; x < size.x; x++)
                {
                    var cell = new Node
                    {
                        obstacle = Convert.ToByte(blockedNodes.Contains(new int2(x, y)))
                    };
                    RequiredExtensions.nodes[GetIndex(new int2(x, y))] = cell;
                }
            }
            ...
            if (Application.isPlaying)
            {
                if (showGrid && size.x * size.y == previousSize)
                {
                    ...
                    for (int x = 0; x < size.x; x++)
                    {
                        for (int y = 0; y < size.y; y++)
                        {
                            Gizmos.color = cells[GetIndex(new int2(x, y))].Obstacle ? Color.grey : Color.white;
